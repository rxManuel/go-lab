...
func TestIsPrimeDeclarativeClassicalRaw(t *testing.T) {
	t.Run("Negative", func(t *testing.T) {
		if got := IsPrimeRaw(-1); got != false {
			t.Errorf("IsPrime(-1) = %v; want false", got)
		}
	})

	t.Run("Zero", func(t *testing.T) {
		if got := IsPrimeRaw(0); got != false {
